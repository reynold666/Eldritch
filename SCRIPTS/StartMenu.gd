...
func _on_BACKARROW_gui_input(event):
...
func _on_CREDITS_gui_input(event):
	if event is InputEventMouseButton:
		if event.pressed:
			$STARTMENU.visible= !$STARTMENU.visible
			$STARTMENU/CREDITS2.visible= !$STARTMENU/CREDITS2.visible
			$CREDITSSCREEN.visible= !$CREDITSSCREEN.visible

func _on_CREDITS_mouse_entered():
	$STARTMENU/CREDITS2.visible=true


func _on_CREDITS_mouse_exited():
	$STARTMENU/CREDITS2.visible=false
